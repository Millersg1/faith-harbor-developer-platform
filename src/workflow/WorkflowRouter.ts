...
export function createWorkflowRouter(
  ...
  /**
   * Creates a traditional workflow.
   */
  router.post("/", (req, res) => {
    const parsed =
  ...
  router.post(
    ...
    (req, res) => {
      const parsed =
        createClientWorkRequestSchema.safeParse(
          req.body,
        );
      ...
      if (!parsed.success) {
        return res.status(400).json({
          error: {
            ...
            message:
              "Client work request validation failed.",
            details:
              parsed.error.flatten(),
          },
        });
