...
describe("Marketing campaigns", () => {
  it("never leaks a campaign across tenants", async () => {
    const repo =
      new PlatformCampaignRepository();
    const now =
  ...
  it("creates a campaign with a budget and advances its stage", async () => {
    const { app, signup } =
      await build();
    ...
    expect(
      created.body.campaign.status,
    ).toBe("planned");
    expect(
      created.body.campaign.budget,
    ).toBe(1000);
    const id =
      created.body.campaign.id;
