...
- **Marketplace preview images** — each of the 17 template systems now shows an
  AI-generated **sample** screenshot on its card (served statically at
  `/marketplace-previews/<id>.jpg`; the list APIs return a `previewImage` path;
  a public static route serves them). The panel states clearly that previews
  are samples and each real site is generated fresh, so a tenant's result is
  unique. Samples were generated by the AI builder and captured headless.
- **Website generator: self-contained output** — strengthened the generator's
  system prompt to forbid external image/background URLs (which rendered blank)
  and require legible colored/gradient backgrounds. Fixes broken hero images on
  generated sites (samples and real tenant sites alike).
